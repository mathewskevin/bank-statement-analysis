...
lookup_filename = os.path.join(os.getcwd(), 'config_lookup.xlsx')
...
class account_checking(account_template):
    ...
    def df_update(self):
        ...
        debit_string = config_variables.debit_string
...
def gen_transactions(credit_outflow, checking_outflow, savings_inflow):
    ...
    transactions = transactions[['Account','Date','Description','Inflow','Outflow','Memo','Category','Month','date_col']]
    return transactions
...
def gen_transactions_distributions(transactions):
    ...
    public_categories = public_categories['Category'].to_list()
    #public_categories = public_categories.loc[:,'Category'].to_list()

    tdist_public = transactions_distribution.loc[transactions_distribution.index.isin(public_categories)].copy()
    ...
    tdist_public = pd.concat([tdist_public, tdist_discretionary])

    # outflow row
    spent_df = tdist_public.loc[[i for i in tdist_public.index if 'Inflow' not in i]].sum()
    spent_df = pd.DataFrame(spent_df).T
    spent_df.index = ['Total | Outflow']
    ...
    inflow_pivot.index = ['Total | Inflow']

    tdist_public.sort_index(inplace = True)
    tdist_public = pd.concat([tdist_public, spent_df])
    tdist_public = pd.concat([tdist_public, inflow_pivot])

    tdist_public.replace(0, float('nan'), inplace=True)

    return transactions_distribution, tdist_public

# write data to excel
def write_excel_file(transactions, tdist_public):
    ...
    def add_distribution_comments(worksheet, dist_df, transactions):
        ...
        def gen_comment_table(col_name, row_name, dist_df, transactions):
            ...
            if row_name == 'Total | Inflow':
                comment_table = comment_table_values.sort_values('date_col', ascending=True)[['Date','Description','Inflow']]
                comment_table['Description'] = comment_table['Description'].apply(short_string)
                inflow_numerator = comment_table['Inflow'].sum()

            elif (row_name == 'Four Walls | Haircuts') or (row_name == 'Four Walls | Rent') or (row_name == 'Abnormal | New'):
                comment_table = comment_table_values.sort_values('date_col', ascending=True)[['Date','Description','Outflow']]
                comment_table['Description'] = comment_table['Description'].apply(short_string)
                inflow_numerator = comment_table['Outflow'].sum()

            elif row_name == 'Total | Outflow':
                comment_table = dist_df[dist_df[col_name]!='None'][col_name]
                comment_table = comment_table.loc[[i for i in comment_table.index if 'Total' not in i]]
                comment_table = pd.DataFrame(comment_table).reset_index()
                comment_table = pd.concat([comment_table ,comment_table['index'].str.split('|', expand=True)[0]], axis=1)
                comment_table.drop('index', axis=1, inplace=True)
                #comment_table.set_index(0, inplace=True)

                comment_table = pd.pivot_table(comment_table, values=[col_name], index=0, aggfunc=np.sum).reset_index()
                inflow_numerator = dist_df.loc[row_name, col_name]
                inflow_denominator = dist_df.loc['Total | Inflow', col_name]
                comment_table['Percent'] = (comment_table[col_name] / inflow_denominator) * 100
                comment_table['Percent'] = comment_table['Percent'].round(2)
                comment_table.columns = ['Category','Spent','% of Inflow']
                ...
                comment_table = comment_table[['Description',col_type,'Count','Average']]
                inflow_numerator = comment_table['Outflow'].sum()

            inflow_denominator = dist_df.loc['Total | Inflow',col_name]
            inflow_percent = round((inflow_numerator / inflow_denominator) * 100, 2)
            inflow_percent = str(inflow_percent) + '% of Inflow'

            return comment_table, inflow_percent
        ...
        for col_name, col_data in dist_df.iteritems():
            for row_name, cell_data in col_data.iteritems():
                ...
                if cell_value != 'None':
                    ...
                    sys.stdout.flush()
                    comment_table, inflow_percent = gen_comment_table(col_name, row_name, dist_df, transactions)
    ...
    worksheet.autofilter("A1:I" + str(transactions.shape[0]))
...
for account_type in data_list:
    ...
    if account_type == 'checking':
        account_mid = account_checking(mid_dataset, config_variables.checking_name)

    # edit credit dataset
    if account_type == 'credit':
        account_mid = account_credit(mid_dataset, config_variables.credit_name)

    # edit savings dataset
    if account_type == 'savings':
        account_mid = account_savings(mid_dataset, config_variables.savings_name)
...
savings_inflow = account_dict.get(config_variables.savings_name).df_inflow
df_inflow = savings_inflow.copy()
credit_outflow = account_dict.get(config_variables.credit_name).df_outflow
checking_outflow = account_dict.get(config_variables.checking_name).df_outflow
...
# write data to excel
write_excel_file(transactions, tdist_public)
