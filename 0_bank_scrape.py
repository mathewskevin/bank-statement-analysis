...
# enter password
user_name = config_variables.BANK_USERNAME
user_pass = config_variables.BANK_PASSWORD

# input variables
site_name = config_variables.site_name # site name
...
profile_loc = config_variables.profile_loc # web browser profile

credit_name = config_variables.credit_name # credit account name
checking_name = config_variables.checking_name # checking account name
savings_name = config_variables.savings_name # savings account name

download_folder = r"/home/kevin/Downloads/"
#database_file = data_folder + '\\database_file.xlsx' # database_file (please enable editing)
src = os.path.join(download_folder, 'transactions.CSV')

#start_date = '8/1/2019' # find latest date in current data
start_date = pd.DataFrame(os.listdir(data_folder))[0].str.split('-',n=2,expand=True)[1].str.split('.',expand=True)[0].sort_values(ascending=False).dropna().iloc[0]
start_date = start_date.split('_')
#start_date_mid = datetime.datetime.strptime(start_date, '%m/%d/%Y')
    ...
    for dates in date_list:
        ...
        while download_button == '':
            try:
                ...
                #download_button = browser.find_element_by_css_selector('button[class=\"btn btn-primary btn-lg account-downloadtransactions-download-button\"]')
                download_button = browser.find_element_by_css_selector('a[id=\"DownloadTransactionsLink\"]')
            except:
                download_button = ''
            ...
            dst = os.path.join(download_folder, output_name)
